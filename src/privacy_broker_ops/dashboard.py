...
def create_app(db_path: Path = DEFAULT_DB_PATH) -> FastAPI:
    ...
    def index() -> str:
        ...
        rows = "".join(
            f"<tr><td>{e.id}</td><td>{e.broker_id}</td><td><a href='{e.url}'>{e.url}</a></td>"
            f"<td>{e.status.value}</td><td>{e.discovered_at}</td><td>{e.last_contact_at or ''}</td>"
            f"<td>{e.data_type}</td><td>{e.note}</td></tr>"
            for e in exposures
        )
        cards = "".join(
            f"<div class='card'><strong>{status.value}</strong><br>{counts.get(status.value, 0)}</div>"
        ...
        <head>
          ...
          <style>
            ...
            .card {{ background: white; border: 1px solid #ddd; border-radius: 8px; padding: .8rem; min-width: 150px; }}
        ...
        <body>
          ...
          <table>
            <thead>
              <tr><th>ID</th><th>Broker</th><th>URL</th><th>Statut</th><th>Découverte</th><th>Dernier contact</th><th>Type</th><th>Note</th></tr>
            </thead>
            <tbody>{rows}</tbody>
          </table>
        </body>
